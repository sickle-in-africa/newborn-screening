...
running_mcnemar <- function(data_country) {
    suppressWarnings(suppressMessages({
        data_country$result_poct_cat <- ifelse(data_country$result_poct %in% c("SS", "AS", "SC"), "SCD", "Non-SCD")
        data_country$result_dbs_cat <- ifelse(data_country$result_dbs %in% c("SS", "AS", "SC"), "SCD", "Non-SCD")

        result_table <- table(data_country$result_dbs_cat, data_country$result_poct_cat)
        ...
        diagnostics_path <- paste0("results/analysis_all/mcnemar_test/mcnemar_test.csv")
        write.table(t(c("result_dbs", "Non-SCD", "SCD")), diagnostics_path, sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE)
        write.table(t(c("result_poct", rep("", ncol(result_df) - 1))), diagnostics_path, sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)
        write.table(result_df, diagnostics_path, sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)
    }))
}
...
cat("./scripts/analysis/analyze_all_sites.R executed successfully.","\n")
